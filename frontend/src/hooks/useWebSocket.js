...
export function useWebSocket() {
  ...
  const subscribeToBus = useCallback((busId, callback) => {
    if (!listeners.current[busId]) listeners.current[busId] = new Set();
    listeners.current[busId].add(callback);
    // ask server to track this bus
    if (ws.current?.readyState === WebSocket.OPEN) {
      ws.current.send(JSON.stringify({ type: 'subscribe', busId }));
    }
    return () => {
      listeners.current[busId]?.delete(callback);
    };
  }, []);
  ...
  return { busLocations, connected, subscribeToBus, sendLocation };
}